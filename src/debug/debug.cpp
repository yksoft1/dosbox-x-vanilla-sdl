...
void DEBUG_WaitNoExecute(void) {
    ...
    CPU_Decoder* oldcore = cpudecoder;
...
static void LogBIOSMem(void) {
    ...
    for (std::vector<RegionAllocTracking::Block>::iterator i=rombios_alloc.alist.begin();i!=rombios_alloc.alist.end();i++) {
...
static void LogDOSKernMem(void) {
    ...
    for (std::list<DOS_GetMemLog_Entry>::iterator i=DOS_GetMemLog.begin();i!=DOS_GetMemLog.end();i++) {
        sprintf(tmp,"%04x     %8lu     ",
